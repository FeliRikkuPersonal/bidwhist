...
export default function CardPlayZone({
    ...
    const runAnimation = async () => {
      ...
      if (animation.type === "PLAY") {
        ...
        if (!fromRef?.current || !toRef?.current) {
          console.warn("❌ Missing refs for PLAY animation", {
            fromRef,
            toRef,
          });
          setTimeout(() => {
      ...
      if (animation.type === "COLLECT") {
        ...
        const myTeam =
          viewerPosition === "P1" || viewerPosition === "P3" ? "A" : "B";
        ...
        for (let card of cardList) {
          ...
          if (!from?.current || !to?.current) {
            console.warn("Missing ref for trick collect animation:", {
              from,
              to,
            });
            continue;
  ...
  useEffect(() => {
    ...
    console.log(
      `Winner Positions: ${bidWinnerPos} Player Position ${viewerPosition}`
    );
  ...
  return (
    <div ref={localRef} className="card-play-zone">
      ...
      ></div>
      <div
        ref={(el) => el && register(`play-east`, { current: el })}
        className="play-slot east"
      ></div>
      <div
        ref={(el) => el && register(`play-west`, { current: el })}
        className="play-slot west"
      ></div>
