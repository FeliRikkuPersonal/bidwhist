...
class HandUtilsTest {
  ...
  void setUp() {
    ...
    sampleHand =
        new ArrayList<>(List.of(twoSpades, aceSpades, jokerB, threeHearts, fiveClubs, kingSpades));

    uptownGame = new GameState("uptown");
    uptownGame.setTrumpSuit(Suit.HEARTS);
    uptownGame.setBidType(BidType.UPTOWN);
    uptownGame.setWinningBid(
        new FinalBid(PlayerPos.P1, 5, false, false, BidType.UPTOWN, Suit.HEARTS));

    downtownGame = new GameState("downtown");
    downtownGame.setTrumpSuit(Suit.CLUBS);
    downtownGame.setBidType(BidType.DOWNTOWN);
    downtownGame.setWinningBid(
        new FinalBid(PlayerPos.P2, 5, false, false, BidType.DOWNTOWN, Suit.CLUBS));
  ...
  void testAllHigherCardsPlayedTrue() {
    ...
    boolean result =
        HandUtils.allHigherCardsPlayed(
            twoSpades, played, hand, Suit.SPADES, BidType.DOWNTOWN, false);
    assertTrue(result);
  }
  ...
  void testHasDiscardSuit() {
    ...
    assertFalse(
        HandUtils.hasDiscardSuit(
            List.of(threeHearts, queenHearts, new Card(Suit.HEARTS, Rank.FIVE)), Suit.HEARTS));
  }
  ...
  void testDetermineTrickWinnerUptown() {
    List<PlayedCard> trick =
        List.of(new PlayedCard(PlayerPos.P1, twoSpades), new PlayedCard(PlayerPos.P2, aceSpades));
