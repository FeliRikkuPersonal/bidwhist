...
public class HandUtils {
   ...
   *
   * <p>Skips cards with null suit (e.g. unassigned jokers during NO_TRUMP). Assumes jokers have
  ...
  /**
   * Returns the best legal card from the hand that can beat the current winning card. If no legal
   * card can beat it, returns null.
   *
   * <p>Rules: - If player has lead suit, they must follow it. - Otherwise, they may cut with trump
   * suit.
   */
  public static Card canBeat(GameState game, List<PlayedCard> currentTrick, List<Card> myHand) {
    ...
    Comparator<Card> cardComparator =
        CardUtils.getCardComparator(game); // 💡 You may need to add this helper

    return myHand.stream()
        .filter(
            card -> {
              if (hasLeadSuit && card.getSuit() != leadSuit) return false;
  ...
  /**
   * Checks whether both opponents of the given player are void in the specified suit.
   *
  ...
  public static boolean isPartnerSuitVoid(GameState game, PlayerPos thisPlayer, Suit targetSuit) {
    if (game == null || targetSuit == null) return false;

    Map<PlayerPos, Map<Suit, Boolean>> voidMap = game.getSuitVoidMap();
    if (voidMap == null || voidMap.isEmpty()) return false;

    // Determine partner position
    PlayerPos partner =
        (thisPlayer.ordinal() + 2) % 4 == 0
            ? PlayerPos.values()[0]
   ...
   * @return true if the hand contains at least one card of the given suit; false otherwise.
   */
    ...
    return trick.stream()
        .max(
            Comparator.<PlayedCard>comparingInt(
                    pc -> {
                      Card c = pc.getCard();
                      if (c.isJoker()) return 3000; // Jokers always win
                      if (trumpSuit != null && c.getSuit() == trumpSuit) return 2000;
                      if (c.getSuit() == leadSuitForComparison) return 1000;
                      return 0;
                    })
   ...
   *
   * <p>Priority is as follows: 1. Return the lowest card that follows the lead suit (if any). 2. If
   * unable to follow suit, return the lowest trump card. 3. If no trump or lead suit cards are
   * available, return the lowest card overall.
   *
   * <p>Rank comparison respects UPTOWN (high wins) and DOWNTOWN (low wins) bid types.
   *
  ...
  public static Card getLowestLegalCard(GameState game, List<PlayedCard> trick, List<Card> hand) {
    ...
    boolean hasLeadSuit =
        hand.stream().anyMatch(c -> leadSuit != null && leadSuit.equals(c.getSuit()));
    ...
    return hand.stream().min(cardComparator).orElse(null);
  }

  /**
   * Returns the lowest-ranked non-trump card that follows the lead suit. If none exists, returns
   * the lowest-ranked non-trump card in hand. If still none exists, returns the lowest trump card.
  ...
  public static Card getLowestLegalNonTrumpCard(
    ...
    Optional<Card> legalNonTrumpLeadSuit =
        hand.stream()
            .filter(
                c ->
                    c.getSuit() != null
  ...
  /**
   * Returns the highest-ranked legal card the player can play in the current trick.
   *
   * <p>If the player has cards in the lead suit, chooses the highest of those. Otherwise, tries to
   * play the highest trump suit card (if any). If neither lead nor trump suits are available,
   * returns the highest overall card.
   *
   * <p>Ranking is based on the game’s bid type: - Uptown: higher rank values are better (Ace high)
  ...
  public static Card getHighestLegalCard(GameState game, List<PlayedCard> trick, List<Card> hand) {
    ...
    return hand.stream().max(Comparator.comparing(Card::getRank, rankComparator)).orElse(null);
  }

  /**
   * Returns the highest-ranked card in the player’s hand.
   *
   * <p>Ranking is determined by the game’s bid type (Uptown or Downtown). In Uptown, higher rank
   * values are better (Ace high). In Downtown, lower rank values are better (2 low).
   *
   * <p>No filtering is done by suit or legality—this purely picks the strongest-ranked card.
   *
   * @param game The current game state (used to determine bid type)
   * @param hand The player's hand
   * @return The card with the highest effective rank, or the first card if hand is empty
   */
  ...
  /**
   * Returns the lowest-ranked card of the specified suit from the given hand. If no cards of the
  ...
  /**
   * Checks if the hand contains a non-trump suit with 2 or fewer cards. This is useful for finding
  ...
  /**
   * Returns the best discard card from hand: Prioritizes suits where the player holds 2 or fewer
   * cards and avoids trump suit. Within that suit, selects the lowest-ranked card.
   *
   * @param game The current game state.
   * @param hand The AI player's hand.
   * @return The selected discard card, or any lowest-ranked card if no discardable suit found.
   */
  public static Card getDiscardCard(GameState game, List<Card> hand) {
    ...
    Map<Suit, List<Card>> nonTrumpSuitGroups =
        hand.stream()
            .filter(c -> c.getSuit() != null && !c.getSuit().equals(trumpSuit))
    ...
    for (Map.Entry<Suit, List<Card>> entry : nonTrumpSuitGroups.entrySet()) {
      ...
      if (cards.size() <= 2) {
        return cards.stream().min(Comparator.comparing(Card::getRank, rankComparator)).orElse(null);
      }
    }
  ...
  /**
   * Returns the partner PlayerPos for the given player. Assumes P1–P3 and P2–P4 are always
   * partnered.
   */
  ...
  /**
   * Finds the next higher card in the same suit as the target card from a given list of cards.
  ...
  /**
   * Returns the list of cards from the hand that are legally playable according to standard
   * trick-taking rules.
   *
   * <p>If the lead suit is present in the player's hand, only cards of that suit are considered
